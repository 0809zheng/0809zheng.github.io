...
**主成分分析(Principal Component Analysis,PCA)**是一种利用线性映射来进行数据降维的线性降维方法。该算法在降维时去除数据的**相关性**，且最大限度保持原始数据的**方差**信息。主成分分析的步骤是对归一化数据的协方差矩阵进行特征值分解(或直接对归一化的数据矩阵进行奇异值分解)。
...
线性降维可以用一个线性变换矩阵$W \in \Bbb{R}^{k×d}$来表示(值得一提的是，**PCA**会对原始数据进行预处理，即先对每一维特征减去其平均值):
...
$$ \mathop{\min}_W  \frac{1}{N} \sum_{n=1}^{N} {(x_n-x'_n)^2} = \frac{1}{N} \sum_{n=1}^{N} {(x_n-W^TWx_n)^2} $$
...
$$ \mathop{\min}_{U,Σ}  \frac{1}{N} \sum_{n=1}^{N} {(x_n-UΣU^Tx_n)^2} \\ = \frac{1}{N} \sum_{n=1}^{N} {(UIU^Tx_n-UΣU^Tx_n)^2} = \frac{1}{N} \sum_{n=1}^{N} {(U(I-Σ)U^Tx_n)^2} $$
...
$$ \mathop{\max}_{U}  \frac{1}{N} \sum_{n=1}^{N} {(UU^Tx_n)^2} = \frac{1}{N} \sum_{n=1}^{N} {x_n^Tuu^Tuu^Tx_n} \\ = \frac{1}{N} \sum_{n=1}^{N} {x_n^Tuu^Tx_n} = \frac{1}{N} \sum_{n=1}^{N} {u^Tx_nx_n^Tu}, \quad  u^Tu = 1 $$
...
对投影差异的解释如下。若向量$x_n - \overline{x}$与向量$u$之间的夹角为$\theta$，则将向量$x_n - \overline{x}$直接投影到向量$u$上的长度为$\|x_n - \overline{x}\|cos \theta$。直接计算两向量内积$(x_n - \overline{x}) \cdot u=\|x_n - \overline{x}\| \|u\| cos \theta$；当$\|u\|=1$时两者等价，因此用内积表示投影差异。
...
$$ \mathop{\max}_{u} J \\ s.t. \quad u^Tu = 1 $$
...
$$ \mathop{\min}_{u} J = \mathop{\min}_{u} \sum_{i=k+1}^{d} {u_i^TSu_i} \\ s.t. u^Tu = 1 $$
