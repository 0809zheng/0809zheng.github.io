...
其中$K$是相似度矩阵(**DPP kernel**)，通常是$N\times N$的实对称半正定方阵。$K_A$是由$A$中元素在$$\mathcal{Y}$$中的标号相对应的元素构成的$K$的**主子式**（**principle minors**, 任选$i$行$i$列的子方阵）。下图给出一个主子式的例子：

![](https://pic.imgdb.cn/item/63296c5616f2c2beb1af7f05.jpg)

对于$$\forall \mathcal{A} \in \mathcal{Y}$$，$$P(A ⊆ Y) = \det(K_A) \in [0,1]$$，因此$K$的所有特征值与主子式都处于$[0,1]$，即$K$是半正定矩阵。$K$也被称为**边缘核**，因为它确定了**DPP**的边缘分布：
...
从上式可以观察到，非对角元素$K_{i,j}=K_{j,i}$表示成对元素之间的负关系度量。$K_{i,j}$值越大，表示$i,j$越不可能同时出现。如果$K_{i,j}$表示$Y$中成对元素之间相似性的度量，则相似程度较高的元素不太可能同时出现。
...
为了对真实数据建模，通过**L-ensemble**来构造边缘核矩阵$K$。具体地，**L-ensemble**通过一个半正定矩阵$L$定义**DPP**：
...
其中$I_{\overline{A}}$是指如果集合$A$包含索引$i$，则$[I_{\overline{A}}]_{ii}=0$；如果不包含索引$i$，则$[I_{\overline{A}}]_{ii}=1$。下面给出一个二阶行列式的例子：
...
如果边缘核矩阵$K$的每一个特征值都在$$\{0,1\}$$中，则对应的**DPP**称为**初等行列式点过程 (elementary DPPs)**。

一个初等**DPP** $$\mathcal{P}^V$$的边缘核矩阵$K^V$的特征值分解可以由一组标准正交向量积$$\{v_n\}_{n\ in V}$$表示：
