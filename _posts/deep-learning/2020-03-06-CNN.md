...
可变形卷积具有任意形状的感受野。当尺寸为$k \times k$的可变形卷积核作用于中心位置$p_0$的局部特征时，通过标准卷积学习$2k^2$个偏移项(分别控制水平和垂直方向的偏移)。偏移项的学习结果为浮点数，因此通过双线性插值计算其对应的像素值。可变形卷积的实现过程可参考[<font color=blue>Deformable Convolution</font>](https://0809zheng.github.io/2022/12/24/deformable.html)。
