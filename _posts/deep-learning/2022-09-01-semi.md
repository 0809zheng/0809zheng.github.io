...
常用的半监督学习方法包括：
- **一致性正则化**：假设神经网络的随机性或数据增强不会改变输入样本的真实标签，如$\Pi$**-Model**, **Temporal Ensembling**, **Mean Teacher**, **VAT**, **ICT**。
- **伪标签**：根据当前模型的最大预测概率为无标签样本指定假标签，如
- 一致性正则化**+**伪标签：既构造无标签样本的伪标签，又同时建立监督损失和无监督损失，如
...
**MixMatch**针对每个无标签样本生成$K$个数据增强的样本$$\overline{u}^{(k)},k=1,...,K$$，然后通过预测结果的平均构造伪标签：

$$ \hat{y} = \frac{1}{K} \sum_{k=1}^K f_{\theta}(\overline{u}^{(k)}) $$

![](https://pic.imgdb.cn/item/63be1cc8be43e0d30e1c9988.jpg)

在此基础上，**MixMatch**结合一致性正则化、熵最小化、**MixUp**增强，以此构造监督损失和无监督损失：
