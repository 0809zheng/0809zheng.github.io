...
$$ \log p(x) = \log \sum_{z}^{} p(x,z) = \log \sum_{z}^{} \frac{p(x,z)}{q(z|x)}q(z|x) \\ = \log \Bbb{E}_{z \text{~} q(z|x)}[\frac{p(x,z)}{q(z|x)}] \geq \Bbb{E}_{z \text{~} q(z|x)}[\log \frac{p(x,z)}{q(z|x)}] $$
...
| 方法 | 重构损失 | 正则化项
| :---: | :---:  | :---:  |
| VAE | $\mathbb{E}_{z \text{~} q(z\|x)} [-\log p(x\|z)]$ <br> $p(x\|z)$:伯努利分布或正态分布 | $KL[q(z\|x)\|\|p(z)]$ <br> $q(z\|x)~\mathcal{N}(\mu,\sigma^{2}),p(z)~\mathcal{N}(0,1)$ |
| [<font color=Blue>CVAE</font>](https://0809zheng.github.io/2022/04/02/cvae.html) <br> 条件VAE | $\mathbb{E}_{z \text{~} q(z\|x,y)} [-\log p(x\|z,y)]$  | $KL[q(z\|x,y)\|\|p(z\|y)]$  |
| [<font color=Blue>WAE</font>](https://0809zheng.github.io/2022/04/04/wae.html) <br> 引入**Wasserstein**距离 | $\Bbb{E}_{x~p(z)}\Bbb{E}_{z \text{~} q(z\|x)} [c(x,p(x\|z))]$ | $\lambda \cdot \mathcal{D}_Z(q(z),p(z))$ <br> $\mathcal{D}_Z$:对抗训练或最大平均差异**MMD**$,p(z)~\mathcal{N}(0,1)$ |
