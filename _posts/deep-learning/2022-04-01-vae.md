...
| [<font color=Blue>HFVAE</font>](https://0809zheng.github.io/2022/04/16/hfvae.html) <br> 隐变量特征分组 | $$\mathbb{E}_{z \text{~} q(z\|x)} [-\log p(x\|z)] + \sum_{i}KL[q(z_{i})\|\|p(z_{i})]  + \alpha KL(q(z,x)\|\|q(z)p(x))$$ <br> $$+\beta \Bbb{E}_{q(z)}[\log \frac{q(z)}{\prod_{g}q(z^g)}-\log \frac{p(z)}{\prod_{g}p(z^g)}]  + \gamma \sum_{g} \Bbb{E}_{q(z^g)}[\log \frac{q(z^g)}{\prod_{j}q(z^g_j)}-\log \frac{p(z^g)}{\prod_{j}p(z^g_j)}]$$   | 
