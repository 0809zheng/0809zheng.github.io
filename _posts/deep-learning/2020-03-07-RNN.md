...
虽然**RNN**理论上可以建立长时间间隔的状态之间的依赖关系，但是由于梯度爆炸或消失问题，实际上只能学习到短期的依赖关系。这个问题称作**长程依赖问题（Long-Term Dependencies Problem）**。
...
**LSTM**网络包括输入门$i$ (**input gate**)、遗忘门$f$ (**forget gate**)，和输出门$o$ (**output gate**)；并引入了一个新的记忆状态$c$（cell state）进行线性的循环信息控制。
