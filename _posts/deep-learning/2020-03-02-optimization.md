...
| [<font color=Blue>LARS</font>](https://0809zheng.github.io/2020/12/15/lars.html)：为每一层参数设置自适应学习率 | $$M_t\text{: 动量}(0) \\ \gamma\text{: 学习率} \\ \mu \text{: 衰减率}(0.9)$$ | $$\begin{align}  M_t &= \mu M_{t-1} + g_t \\ θ_t^{(i)}&=θ_{t-1}^{(i)}-\gamma \frac{|| θ_{t-1}^{(i)} ||}{|| M_t^{(i)} ||} M_t^{(i)}, \text{for all }i \in [L] \end{align}$$   |
