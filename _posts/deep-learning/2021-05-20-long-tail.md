...
在**pytorch**中，可以为`DataLoader`传入采样器`sample`，从而实现不同类别的重采样（本质是调整样本权重）：
```python
...
最小化上述损失函数，即使得所有非目标类的**logits**得分$z_{i\ne c}$均小于目标类的**logits**得分$z_c$。上述优化过程会对所有非目标类产生负样本梯度，抑制这些类别的预测过程。**tail**类样本作为非目标类样本的频率更高，因此抑制程度更强。
...
$$ -\log(\frac{\exp(z_c-m_c)}{\sum_{i=1}^{C} \exp(z_i-m_i)}) = -\log(\frac{\exp(-m_c)\exp(z_c)}{\sum_{i=1}^{C} \exp(-m_c)\exp(z_i)}) $$
