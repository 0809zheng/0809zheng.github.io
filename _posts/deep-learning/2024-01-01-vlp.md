...
**VinVL**通过改进目标检测模型来提供更丰富的图像视觉表示。**VinVL**基于**ResNeXt-152 C4**架构，在预训练阶段使用掩码标记损失和三元组对比损失。
...
**BEiT-3**将图像视为一种“外语”，并通过掩码建模的方式在单模态（图像、文本）和多模态（图像-文本对）数据上进行预训练。这种方法不仅简化了预训练任务，还使得模型能够高效扩展。**BEiT-3**使用共享的**Multiway Transformer**作为骨干网络，其形式与**VLMo**相似。
...
**GLIP**使用**BERT**作为**Text Encoder**，**DyHead**作为**Visual Encoder**，之后通过交叉注意力机制融合文本和区域特征，融合后文本和区域特征会分别经过**BERTLayer**和**DyHead Module**进一步提取特征，最后通过对比学习进行特征对齐。
