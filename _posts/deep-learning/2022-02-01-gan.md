...
- 改进目标函数：选用**f**散度(如**f-GAN**, **RGAN**)、均方误差(如**LSGAN**)、**Wasserstein**距离(如**WGAN**, **WGAN-GP**, **SN-GAN**, **GN-GAN**, **GraN-GAN**, **c-transform**)、**W**散度(如**WGAN-div**)
- 改进网络结构：使用卷积神经网络(如**DCGAN**, **ResNet**)、引入编码器(如**VAE-GAN**, **BiGAN**)
...
对于[分布散度](https://0809zheng.github.io/2020/02/03/kld.html)，当两个概率分布没有重合时，散度取值通常是固定值，从而导致优化梯度为0。在实践中可以选择具有更平滑的值空间的分布度量指标。
