...
- 改进目标函数：选用**f**散度(如**f-GAN**)、**Wasserstein**距离(如**WGAN**, **WGAN-GP**)
- 改进网络结构：**DCGAN**
- 改进优化过程
...
| [<font color=Blue>f-GAN</font>](https://0809zheng.github.io/2022/02/07/fgan.html) <br> f散度 | $$ (D^*, G^*) \leftarrow \mathop{\min}_{G}\mathop{\max}_{D \in f'(\Bbb{D})}  \Bbb{E}_{x \text{~} P_{data}(x)}[D(x)]- \Bbb{E}_{x \text{~} P_{G}(x)}[f^*(D(x))]  $$ |
| [<font color=Blue>Wasserstein GAN <br> (WGAN)</font>](https://0809zheng.github.io/2022/02/04/wgan.html) <br> W距离 | $$ \begin{aligned} (D^*, G^*) & \leftarrow \mathop{ \min}_{G} \mathop{ \max}_{D,\|D\|_L \leq K} \Bbb{E}_{x \text{~} P_{data}(x)}[D(x)]-\Bbb{E}_{x \text{~} P_{G}(x)}[D(x)]  \end{aligned} $$ |
| [<font color=Blue>Wasserstein GAN-gradient penalty <br> (WGAN-GP)</font>](https://0809zheng.github.io/2022/02/06/wgangp.html) <br> 梯度惩罚 | $$ \begin{aligned} D^* \leftarrow &\mathop{ \max}_{D} \Bbb{E}_{x \text{~} P_{data}(x)}[D(x)]-\Bbb{E}_{x \text{~} P_{G}(x)}[D(x)] \\ & - λ \Bbb{E}_{x \text{~} \epsilon P_{data}(x) + (1-\epsilon)P_{G}(x) }[(\|\| \nabla_xD(x) \|\| -1)^2]  \\ G^* \leftarrow & \mathop{ \min}_{G}  -\Bbb{E}_{x \text{~} P_{G}(x)}[D(x)] \end{aligned} $$ |
