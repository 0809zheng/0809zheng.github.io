...
| [<font color=Blue>Designing GAN</font>](https://0809zheng.github.io/2022/03/07/design.html) <br> 在对偶空间构造目标函数 | $$ \mathop{\min}_{G} \mathop{\max}_{D \in \Omega} \Bbb{E}_{x\text{~}P_{data}(x)}[ \phi(D(x))]+ \Bbb{E}_{x\text{~}P_{G}(x)}[\psi(D(x))] $$ |
...
https://0809zheng.github.io/2021/03/02/transgan.html
...
### ⚪ [<font color=Blue>Cascading Rejection</font>](https://0809zheng.github.io/2022/04/23/cascading.html)
...
**Pix2Pix**的生成器采用**UNet**网络，把一种类型的图像$x$转换为另一种类型的图像$\hat{y}=G(x)$，损失函数包括判别损失和**L1**重构损失。判别器设计为**PatchGAN**，同时接收两种类型的图像$(x,y)$，判断其是否为真实图像，损失函数包括判别损失。
...
### ⚪ [<font color=Blue>Toward Multimodal Image-to-Image Translation</font>](https://0809zheng.github.io/2022/03/18/bicyclegan.html)
...
[<font color=Blue>StarGAN: Unified Generative Adversarial Networks for Multi-Domain Image-to-Image Translation</font>](https://0809zheng.github.io/2022/03/19/stargan.html)：(arXiv1711)
...
- [<font color=Blue>Simple yet Effective Way for Improving the Performance of GAN</font>](https://0809zheng.github.io/2022/04/23/cascading.html)：(arXiv1911)通过级联抑制方法增强GAN的判别器。
