...
假设真实数据具有概率分布$$P_{data}(x)$$，**GAN**的目的是学习真实分布的一个近似分布$$P_G(x)$$。**GAN**没有显式地定义$$P_G(x)$$，而是使用一个**生成器generator**学习一个数据分布，使用一个**判别器discriminator**衡量生成分布和数据的真实分布之间的差异。
...
下面求判别器$D$的最优值$D^{\*}$，注意到积分不影响最优值的取得，因此计算被积表达式的极值$$\frac{\partial L(G,D)}{\partial D} = 0$$，得：
...
其中$D_{JS}$表示[<font color=blue>JS散度</font>](https://0809zheng.github.io/2020/02/03/kld.html)。因此当判别器$D$取得最优时，**GAN**的损失函数衡量了真实分布$$P_{data}(x)$$与生成分布$$P_G(x)$$之间的**JS散度**。若生成器$G$也取得最优值，则损失函数取得最小值为$-2\log 2$。

在**GAN**的交替训练步骤中，先优化判别器再优化生成器，并且判别器的更新次数通常更多一些；这样做的目的也是为了让判别器先取得局部最优，从而让生成器的优化过程趋近于**JS**散度的最小化过程。

另一方面，传统的基于**极大似然估计(maximum likelihood estimation)**的生成模型本质上是在最小化真实分布$$P_{data}(x)$$与生成分布$$P_G(x)$$之间的[<font color=blue>KL散度</font>](https://0809zheng.github.io/2020/02/03/kld.html)：
...
由于**JS**散度相比于**KL**散度具有对称性、平滑性等优点，因此通常任务**GAN**相比于传统的生成模型能够取得更好的表现。
...
# ⚪ 参考文献
- [<font color=Blue>Random Erasing Data Augmentation</font>](https://0809zheng.github.io/2021/11/26/erasing.html)：(arXiv1708)Random Erasing：随机擦除数据增强。
- [<font color=Blue>mixup: Beyond Empirical Risk Minimization</font>](https://0809zheng.github.io/2020/06/26/mixup.html)：(arXiv1710)mixup：样本对插值的经验风险最小化。